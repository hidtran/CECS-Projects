...
int maint(int argc, char* argv[]) {
   ...
   *A class defines a set of operations called "methods" that can be used
   with the "instances of the class (object)".* 
   ...
   */
   ...
   cout << s.find(',') /* gets first index of the character ',' */ << endl; // < 0 indicates a character doesn't found.
									   // Can also use s.find(',', starting position to look for)

   // extract a substring. first parameter: start index. second: length.
   string sub = s.substr(5, s.size() - 5); // s.substring(starting index, length of the string(size of string - starting index))
   cout << sub << endl; 
