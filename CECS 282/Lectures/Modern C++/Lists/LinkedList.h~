...
public:
   ...
   */
   class LinkedListIterator { // lowercase by expectation.
      Node *mCurrent;	// Points to current node
      LinkedList<TData> *mList;	// Points to the head node itself
      friend class LinkedList < TData >;
   ...
   public:
      ...
      TData &operator*() {
         return mCurrent->mData;	//	Return current data it points to
      }
      ...
      TData &operator->() {
		...
		//Make a copy of itself, and return a copy of itself
      LinkedListIterator operator++(int) {// postincrement
         ...
         if (mCurrent == mList->mHead->mPrev.get())	// head -> prev.get() is at it tails
            mCurrent = nullptr;	//	Set to null
         else
		...
		//	Modify itself and return itself
   ...
   typedef LinkedListIterator iterator; // Represent as a iterator
