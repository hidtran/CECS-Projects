...
public class Tree {
	...
	*/
	...
	private int getDepth(Node current)
	{
		...
		{
			int left = getDepth(current.left);	//Count tree left side
			int right = getDepth(current.right);	///Count tree right side
			if(left > right)
			{
				return left+1;
			}
	...
	private int getSize(Node current)
	{
		...
		{
			length = 1 + getSize(current.left) + getSize(current.right); //Recursive call for tree size
		}
		return length;
	}
